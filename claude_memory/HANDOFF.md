...
comparison"; **exact reproduction recipe: [REPRO_1H_DEADLINE.md](REPRO_1H_DEADLINE.md)**,
table regenerated by `python3 scripts/report_eval2.py`):
**Reproduced 2026-08-27/28** (run 2, four arms, exp_notes F7c): surrogate arms 1.7k median vs
replay 15.6k; first 3 iterations byte-identical across arms (shared seed); cluster ran 22–60 %
slower than in July for identical configs — compare within a run only.
**eval4 (2026-08-29, clean-shutdown harness, F7d):** 11–15 evaluations/h instead of 7–12; surrogate
arms still 3.5–5.6k median vs replay 20.2k on every seed — more iterations do not rescue them.
